...
export class CompileResult {
    ...
    get manifest_json() {
        ...
        let deferred1_1;
        try {
            const ret = wasm.compileresult_manifest_json(this.__wbg_ptr);
            deferred1_0 = ret[0];
            deferred1_1 = ret[1];
     ...
     */
    get wasm_bytes() {
        const ret = wasm.compileresult_wasm_bytes(this.__wbg_ptr);
 ...
 * @param {string} text
 * @param {boolean} simd128
 * @param {boolean} relaxed_simd
 * @returns {CompileResult}
 */
export function compile_with(text, simd128, relaxed_simd) {
    const ptr0 = passStringToWasm0(text, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.compile_with(ptr0, len0, simd128, relaxed_simd);
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return CompileResult.__wrap(ret[0]);
 ...
 * @returns {Uint8Array}
 */
export function relaxed_simd_probe() {
    const ret = wasm.relaxed_simd_probe();
    var v1 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
    wasm.__wbindgen_free(ret[0], ret[1] * 1, 1);
    return v1;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('whlo_web_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
